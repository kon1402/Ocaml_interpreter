...
let decode_sequence s =
  let rec extract s idx =
    ...
    if s = 1 then [] (* If remaining s is 1, stop *)
    else if exponent = 0 then extract s (idx + 1)
    else exponent :: extract (s / (pow prime exponent)) (idx + 1)
  in
  extract s 0
...
let to_string s =
  ...
  let rec format_list = function
    | [] -> ""
    | [x] -> string_of_int x
    | x::xs -> string_of_int x ^ "; " ^ format_list xs
  in
  "[" ^ format_list seq ^ "]"
