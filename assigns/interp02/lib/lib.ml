...
(*function that desugars prog -> expr *)
      ...
      let desugared_value =
        ...
        | _ ->
            List.fold_right
              ...
              (desugar_expr decl.value)
      in
      let desugared_body = desugar rest in
      Let {
        is_rec = decl.is_rec;
        name = decl.name;
        ty = decl.ty;
    ...
    | App (e1, e2) ->
        ...
        (match v1 with
         ...
         | VClos { name = Some f; arg; body; env = clos_env } ->
             ...
             let env' = Stdlib320.Env.add f closure clos_env in
             let env'' = Stdlib320.Env.add arg v2 env' in
             eval_env env'' body
